...
## The three seams (ports in `packages/core/src/contracts/adapters/`, durable drivers in core)

| Seam                       | Token                | For                              | Driver (production AND test)                                              |
| -------------------------- | -------------------- | -------------------------------- | ------------------------------------------------------------------------- |
| Inter-module domain events | `MESSAGE_BROKER`     | one module reacts to another     | Redis Streams (`REDIS_URL`); RabbitMQ/Kafka via overlay                   |
| Background jobs            | `JOB_QUEUE`          | durable/retryable/scheduled work | BullMQ + Redis (`REDIS_URL`)                                              |
| Client push                | `REALTIME_TRANSPORT` | SSE/WS to the browser            | `InProcessRealtimeTransport`; managed vendor (Ably/GetStream) via overlay |
