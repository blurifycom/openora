...
rationale (multi-currency exponents, crypto, wire-format precedent): ADR-0029. Lint error
`oss-module-shape/no-float-money`bans`real`/`float`/`doublePrecision` in schema files.
