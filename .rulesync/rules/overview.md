...
Canonical brief for AI agents (Claude Code, GitHub Copilot, Gemini, Cursor, etc.) working on this repo. Humans read this too. Every per-tool instruction file (`AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, `.github/copilot-instructions.md`) plus the subagent and command mirrors are generated by [rulesync](https://github.com/dyoshikawa/rulesync) from the single source in `.rulesync/` (`rules/`, `subagents/`, `commands/`, `mcp.json`). Edit the source under `.rulesync/`, then run `pnpm sync:agents`. Do not hand-edit the generated files.
