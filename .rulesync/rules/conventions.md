...
- Names carry units/intent: `delayMs`, `maxRetries`, `amountCents` - never bare `delay`, `max`, `price`.
...
- **Never `!` non-null assertions** - narrow or restructure so the value is provably present. A `!` silences the exact check that would catch a null.
