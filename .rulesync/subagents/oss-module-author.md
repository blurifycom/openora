...
This creates `packages/addons/<name>/` as a standalone `@oss-addons/<name>` package with all required files and registers the plugin in `extensions.config.ts`. Never write files from scratch - fill in the `// AGENT: implement here` markers the scaffolder leaves.
...
- `pnpm verify` exits 0.
- Add-on registered in `extensions.config.ts`; for core add-ons, contract slice composed into `@oss/orpc-contract`; for gated add-ons, contract slice in `src/contract/`.
