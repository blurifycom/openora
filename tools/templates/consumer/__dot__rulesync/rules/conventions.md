...
- `pnpm typecheck` + `pnpm lint` (oxlint) + `pnpm test:unit` is the fast gate; `pnpm boundaries` (dependency-cruiser) is the whole-graph boundary/cycle check, also run by the pre-commit hook and CI.
...
- Agent rules are generated from `.rulesync/` via `pnpm sync:agents` - never hand-edit a generated file.
