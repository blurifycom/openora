...
- Last review marker (`review` skill §2a): `gh api "repos/{{gitRemotePath}}/issues/<n>/comments" --jq '[.[] | select(.body | contains("<!-- review:sha=")) | select(.author_association == "OWNER" or .author_association == "MEMBER" or .author_association == "COLLABORATOR")] | last | .body' | grep -o 'review:sha=[0-9a-f]*'`. The `author_association` filter is load-bearing: a PR author or outside contributor can post a comment containing the marker text for any SHA they like, and without the filter that comment would be trusted as a completed review.
