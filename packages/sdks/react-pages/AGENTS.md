...
@oss/react-pages            ← this package. Renders via useUI(); adapter-agnostic.
...
```tsx
import { defineUIPlugin, defineSlotFill, SLOTS } from '@oss/react-pages';
...
```tsx
import { Slot, SLOTS } from '@oss/react-pages';
...
- Don't import from the barrel (`@oss/react-pages`) inside this package - use relative paths; the barrel pulls `next/navigation` and would create cycles.

## Build

```bash
pnpm -F @oss/react-pages build       # tsc -> dist/ (ESM)
pnpm -F @oss/react-pages typecheck
```
