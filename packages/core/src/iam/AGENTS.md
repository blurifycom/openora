...
- Super-admin = a role with `isSuperAdmin=true`; the resolver returns ALL grants for its holders. The bootstrap admin (`user.role='admin'`, no assignment row) also counts via the static fallback, so it can never be locked out.
...
`DEFAULT_ADMIN_ROLES` (`seed/default-admin-roles.ts`) is the canonical spec; `seedRoles` is a convergent upsert keyed on `key` - re-seeding reconciles names/levels back to the spec but does not drop grants removed from it. Roles are seeded by script, never by migration.
