...
| Add permission resources     | Edit `packages/core/src/server/auth/permissions.ts` `statement` - catalog and validation derive from it |
...
- Import identity's service or any other add-on's root entry.
