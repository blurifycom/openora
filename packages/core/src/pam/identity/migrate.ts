...
/** Apply the identity module migrations (idempotent: drizzle skips already-recorded ones). */
export function migrate(databaseUrl?: string) {
  return runMigrations({
    ...
    // user.email GIN trgm index (admin substring search) needs the pg_trgm extension.
    extensions: ['pg_trgm'],
