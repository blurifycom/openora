...
// These stub routes only ever run adminGuard.assert() then throw NOT_IMPLEMENTED - no DB
// row is ever read or written, so a mocked DrizzleService is enough (no real Postgres).
function routerWith(guard: AdminGuard) {
  ...
  return createWalletRouter(service, guard, makeAuditWriter(), paymentProviders);
}
