...
Manages user balances, deposits, and withdrawals. Ships with `MockPaymentAdapter` as the default `PAYMENT_ADAPTER` binding - a synthetic PSP that returns `{ externalId, status: 'completed' }`. Emits domain events on completed transactions. Every user gets one wallet per system; money columns are Postgres `NUMERIC` via drizzle `decimal()` (`wallet.balance`, `wallet_transaction.amount`, `auto_withdrawal_rule.threshold`) - a decimal STRING in TS, never a float, never a scaled integer. Balance mutations run as SQL numeric arithmetic (`sql\`${wallet.balance} + ${amount}::numeric\``); see ADR-0029.
...
- **Ports consumed**: `ADMIN_USER_DIRECTORY` (`lookupPlayers` for queue enrichment + auto-approval KYC), `ADMIN_GUARD` (admin route authz), `PLAYER_TAGS` (optional; auto-approval risk-flag exclusion, bound by the `tag` module), `AUDIT_WRITER` (required; auto-approval + rule-change audit trail - the module `dependsOn: ['identity', 'audit']`).
