...
CREATE UNIQUE INDEX "wallet_deposit_address_address_network_currency_idx" ON "wallet_deposit_address" USING btree ("address","network","currency") WHERE "wallet_deposit_address"."tag" IS NULL AND "wallet_deposit_address"."network" IS NOT NULL;--> statement-breakpoint
CREATE UNIQUE INDEX "wallet_deposit_address_user_id_currency_idx" ON "wallet_deposit_address" USING btree ("user_id","currency") WHERE "wallet_deposit_address"."network" IS NULL;