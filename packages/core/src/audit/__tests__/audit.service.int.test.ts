...
describe('AuditService.recordEventsInTransaction() (real PG)', () => {
  it('inserts a whole batch with a valid, contiguous hash chain', async () => {
    const svc = makeService();

    const rows = await db.drizzle.db.transaction((tx) =>
      svc.recordEventsInTransaction(
        tx,
        Array.from({ length: 25 }, (_, i) => ({
          actorType: 'admin' as const,
          actorId: randomUUID(),
          action: 'compliance.game-geo-rule.upserted',
          resourceType: 'game-geo-rule',
          resourceId: randomUUID(),
          before: null,
          after: { gameId: randomUUID(), countryCode: 'DK', index: i },
        })),
      ),
    );
  ...
  it('chains the first batched row onto whatever record() last wrote, and verifies end to end', async () => {
    const svc = makeService();
    const solo = await svc.record({
      actorType: 'system',
      action: 'identity.user.registered',
      resourceType: 'identity',
    });
    ...
    const rows = await db.drizzle.db.transaction((tx) =>
      svc.recordEventsInTransaction(
        ...
        Array.from({ length: 5 }, () => ({
          actorType: 'admin' as const,
          action: 'compliance.game-geo-rule.deleted',
          resourceType: 'game-geo-rule',
          resourceId: randomUUID(),
          before: { reason: 'was restricted' },
          after: null,
        })),
      ),
    );

    expect(rows[0]?.prevHash).toBe(solo.hash);
    expect(rows.every((r) => r.seq > solo.seq)).toBe(true);
    expect(await svc.verifyChain()).toEqual({ valid: true });
  });
  ...
  it('is a no-op for an empty batch', async () => {
    ...
    const rows = await db.drizzle.db.transaction((tx) => svc.recordEventsInTransaction(tx, []));

    expect(rows).toEqual([]);
    expect(await db.drizzle.db.select().from(auditLog)).toHaveLength(0);
  });

  it('chunks past the bind-parameter limit and still verifies as one chain', async () => {
    const svc = makeService();

    const rows = await db.drizzle.db.transaction((tx) =>
      svc.recordEventsInTransaction(
        tx,
        Array.from({ length: AuditService.INSERT_CHUNK_SIZE + 50 }, () => ({
          actorType: 'admin' as const,
          action: 'compliance.game-geo-rule.upserted',
          resourceType: 'game-geo-rule',
          resourceId: randomUUID(),
          before: null,
          after: { countryCode: 'DK' },
        })),
      ),
    );

    expect(rows).toHaveLength(AuditService.INSERT_CHUNK_SIZE + 50);
    expect(await svc.verifyChain()).toEqual({ valid: true });
  }, 30_000);

  it('routes a bulk write through mapEventToRecord, matching the single-event shape exactly', async () => {
    const svc = makeService();
    ...
    const payload = {
      ...
      before: null,
      after: { id: randomUUID(), gameId, countryCode: 'DK', reason: 'bulk restriction' },
      actorId: randomUUID(),
      ip: null,
      userAgent: null,
    };

    const [viaBatch] = await db.drizzle.db.transaction(async (tx) =>
      svc.recordEventsInTransaction(tx, [
        await mapEventToRecord('compliance.game-geo-rule.upserted', payload),
      ]),
    );
    const viaSingle = await mapEventToRecord('compliance.game-geo-rule.upserted', payload);

    expect(viaBatch).toMatchObject({
      actorType: viaSingle.actorType,
      actorId: viaSingle.actorId,
      action: viaSingle.action,
      resourceType: viaSingle.resourceType,
      resourceId: viaSingle.resourceId,
      before: viaSingle.before,
      after: viaSingle.after,
      result: viaSingle.result,
    });
  });
});
