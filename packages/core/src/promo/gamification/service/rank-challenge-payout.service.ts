...
export class RankChallengePayoutService {
  ...
  private async settleOne(tx: DrizzleTx, claimId: Uuid): Promise<RankChallengeWon | null> {
    ...
    const cashGrantId: string | null = null;
    if (claim.cashAmount !== null && !restricted && this.wallet) {
      ...
      if (!credited.ok) {
        this.logger.error(
          { userId: claim.userId, tierId: claim.tierId, reason: credited.reason },
          'rank challenge cash credit failed',
        );
      }
    } else if (claim.cashAmount !== null && restricted) {
      this.logger.warn(
    ...
    await tx
      ...
      .set({ settledAt: new Date(), outcome: restricted ? 'restricted' : 'granted', cashGrantId })
      .where(eq(promoRankChallengeClaim.id, claimId));
