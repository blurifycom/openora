...
`recipientCount` before the live recipient list is reduced. If fewer users are available, the same
per-recipient amount is paid to each selected user; the unused remainder stays with the sender. For
example, `100` requested for 5 users fixes `20` per recipient; if only 4 are available, 4 users get
`20` and only `80` is debited. `perRecipient` and `totalDistributed` (`perRecipient * actual
...
actually left the sender's wallet. The pre-transaction `maxAmount`/`minAmount`/`amountUnits` limit
checks still validate against the player-typed `input.amount`; `maxRecipients` validates the
requested count. `player_rain` is a new header row per rain event (nothing was persisted before
