...
Each row in `chat_command_config` holds `enabled`, `label`, `description`, and a `config` jsonb column (`maxAmount`, `maxRecipients`). The service checks the row before dispatching — a missing or disabled row throws `CommandDisabledError` (404). Seed data lives in `seed/index.ts` (`seedChatCommands`), called from `tools/db/seed.ts`. Four default commands: `mention`, `profile`, `gift`, `rain`.
...
- `AUDIT_WRITER` — direct `record()` after each gift/rain for the regulatory trail.
- `REALTIME_TRANSPORT` — `getOnlineUserIds(channel)` for rain recipient discovery.
