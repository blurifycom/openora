...
describe('PageQuerySchema', () => {
  it('defaults to page 1 / limit 50 when omitted', () => {
    expect(PageQuerySchema.parse({})).toEqual({ page: 1, limit: 50 });
  });
  ...
  it('caps limit at 50 and floors page at 1', () => {
...
describe('paginated', () => {
  it('wraps an item schema in the canonical envelope', () => {
    ...
    const value = { items: [{ id: 'a' }], total: 1, page: 1, limit: 50 };
    expect(schema.parse(value)).toEqual(value);
  });

  it('rejects a non-integer total', () => {
    const schema = paginated(z.string());
    expect(schema.safeParse({ items: [], total: 1.5, page: 1, limit: 50 }).success).toBe(false);
  });
});
