...
export const translation = pgTable('translation', {
  id: text('id').primaryKey().$defaultFn(() => createId()),
  localeId: text('localeId').notNull().references(() => locale.id, { onDelete: 'cascade' }),
  namespace: text('namespace').notNull(),
  key: text('key').notNull(),
  value: text('value').notNull(),
  updatedAt: timestamp('updatedAt').notNull().$onUpdateFn(() => new Date()),
}, (t) => [
  uniqueIndex('translation_localeId_namespace_key_key').on(t.localeId, t.namespace, t.key),
]);
