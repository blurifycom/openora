...
- Read another module's tables by importing them via the subpath (`import { user } from '@oss/modules/platform/identity/schema'`, `import { wallet, walletTransaction } from '@oss/modules/player/wallet/schema'`) and querying with `DrizzleService` - no casts needed
...
4. Run `pnpm regen && pnpm verify --filter @oss/module-admin-console`
