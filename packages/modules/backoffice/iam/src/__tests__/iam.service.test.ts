...
function makeDrizzle(overrides: Record<string, unknown> = {}) {
  ...
  return { db: chain } as unknown as import('@oss/db').DrizzleService;
}

function makeEvents() {
  return { emit: vi.fn(), on: vi.fn() } as unknown as import('@oss/core').EventBus;
}

function makeEmail() {
  return {
    send: vi.fn().mockResolvedValue(undefined),
  } as unknown as import('@oss/adapters').SendEmailPort;
}
...
describe('DbAdminPermissionResolver', () => {
  ...
  it('returns grants when user has an assignment', async () => {
    ...
    const drizzle = { db: chain } as unknown as import('@oss/db').DrizzleService;
    const resolver = new DbAdminPermissionResolver(drizzle);
