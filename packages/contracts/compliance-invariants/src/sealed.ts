import { createSealedToken, type SealedToken } from '@oss/adapters';

/**
 * Canonical list of regulator-mandated services that operators MAY NOT override.
 *
 * Each sealed token backs an invariant that lives in `@oss/platform/compliance`
 * (or its sibling sealed services). Plugins may *observe* via events but never
