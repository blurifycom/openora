...
// Centralized, schema-validated casino configuration. A downstream operator
// declares one of these and passes it to `createApp({ casino })`; it is the single
// declarative home for the things an operator (or their agent) sets to shape a
// casino - currencies, jurisdictions, limits, provider selection, branding.
...
export const CasinoConfigSchema = z
  .object({
    ...
    // Currencies the casino transacts in; the first is the default.
    currencies: z.array(CurrencyCodeSchema).min(1),
    // Jurisdictions the casino is licensed to operate in.
...
export type CasinoConfigInput = z.input<typeof CasinoConfigSchema>;

/**
 * Validate and normalize a casino config. Throws a precise, agent-legible error
 * (listing the offending path) when the config is malformed - fail-fast at the
 * boundary instead of a deep runtime surprise.
 */
export function defineCasinoConfig(config: CasinoConfigInput): CasinoConfig {
  const result = CasinoConfigSchema.safeParse(config);
  if (!result.success) {
    const issues = result.error.issues
      .map((i) => `  - ${i.path.join('.') || '(root)'}: ${i.message}`)
      .join('\n');
    throw new Error(`Invalid casino config:\n${issues}`);
  }
  return result.data;
}

/** DI token to inject the active CasinoConfig into services/adapters. */
export const CASINO_CONFIG = Symbol('CASINO_CONFIG');
