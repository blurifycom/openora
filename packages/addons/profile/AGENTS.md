...
Player-facing self-profile (core, default build). Owns the `player` table - the
...
registration analytics) is a separate gated add-on package,
`@oss-addons/player-management`, which reads this add-on's `player` table via the
`@oss-addons/profile/schema` subpath. See ADR-0021.
...
- Admin/operator player operations belong in the add-on PAM package, never here.
