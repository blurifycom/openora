...
/**
 * E2E for player-tag lifecycle: self_excluded auto-assign/remove on the RG self-exclusion
 * lifecycle, automatic multi_account + bonus_abuser tagging from identity login signals, the
...
async function registerAndMaterializePlayer(honoApp: TestApp['app'], email: string) {
  const registerRes = await honoApp.request('/identity/register', {
    ...
    body: JSON.stringify({ email, password: 'password123', name: 'E2E Player' }),
  });
  if (!registerRes.ok) {
