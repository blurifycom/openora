...
import { walletBalance, walletTransaction } from '@openora/core/wallet/schema';
...
const gift = (userId: string, amount: string, providerRefId = randomUUID()) =>
  drizzle().transaction((tx) =>
    app.container.get(WALLET_COMMANDS).credit(tx, {
      ...
      currency: 'USD',
      type: 'gift',
      providerRef: { providerName: 'chat', providerRefId },
    }),
  );
...
describe('money the platform gifts', () => {
  ...
  it('writes no wallet_transaction row, since real balance never moved', async () => {
    const { client, userId } = await registerAndMaterializePlayer(app, {
      email: `gift-no-ledger-${randomUUID()}@example.test`,
    });
    await deposit(client, '10');
    await gift(userId, '5');
    ...
    const rows = await drizzle()
      ...
      .where(eq(walletTransaction.type, 'gift'));
    expect(rows).toHaveLength(0);
  });

  it('refuses a gift with no stable caller reference to dedupe a retry against', async () => {
    const { client, userId } = await registerAndMaterializePlayer(app, {
      email: `gift-no-ref-${randomUUID()}@example.test`,
    });
    await deposit(client, '10');
