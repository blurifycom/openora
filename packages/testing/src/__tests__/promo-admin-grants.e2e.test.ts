...
describe('support forfeiting a bonus', () => {
  ...
  it('lets exactly one of two concurrent forfeits win, with one ledger entry and an honest refusal', async () => {
    const { userId } = await registerAndMaterializePlayer(app, {
      email: `forfeit-race-${randomUUID()}@example.test`,
    });
    const grantId = await grantBonus(userId, '60');

    const [first, second] = await Promise.all([
      admin.post(`/backoffice/promo/grants/${grantId}/forfeit`, { note: 'Race attempt one' }),
      admin.post(`/backoffice/promo/grants/${grantId}/forfeit`, { note: 'Race attempt two' }),
    ]);
    ...
    expect(await forfeitRows(grantId)).toHaveLength(1);
    const refusals = await refusalRows(grantId);
    expect(refusals).toHaveLength(1);
    // The refusal has to say what the grant actually is by the time it was refused, not what it
    // was on the way in - otherwise the loser of the race audits a live grant as still active.
    expect(refusals[0]?.after).toMatchObject({ status: 'forfeited' });
  });
