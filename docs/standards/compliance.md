...
- An admin may set a limit outright, without the cool-down, but only to create a first limit or lower an existing one. A raise is refused server-side (ADR-0037): imposing a limit is the compliance function, raising the one a player set for themselves is weakening their protection. Keep the override permissioned, reason-bearing and audited under the admin's own actor.
