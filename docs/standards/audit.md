...
- Audit rows are append-only, enforced by a database trigger that denies UPDATE and DELETE on `audit_log`. Never add update or delete behavior, and never invent a topic outside `domainEventSchemas`.
