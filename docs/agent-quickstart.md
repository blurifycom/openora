...
This creates `packages/domains/<group>/<name>/` (a folder inside the single `@openora/modules` package - NOT its own package) with all required files and registers it in `extensions.config.ts`.
...
If a shared schema exists in `@openora/contracts/shared-schemas`, re-export it instead of duplicating.
...
- Importing from another module directly - use events or read its tables via the `@openora/modules/<group>/<name>/schema` subpath. Both boundary gates reject it: the oxlint `oss-boundaries/*` plugin (per-edit, specifier strings) and the whole-graph `pnpm boundaries` gate (catches transitive / re-export / dynamic-import / relative-path dodges too).
