...
- **A plugin-contributed UI extension** (column, tile, nav item, section, ribbon, route - not a core page) -> use a client-side `defineUIPlugin` instead of editing react-pages. Slot fills support `visibleWhen` / `requiresPermission` / `brandScope` / `featureFlag` for declarative gating. See ADR-0006, ADR-0013, and the `@oss/example-vip-tier` reference plugin.
