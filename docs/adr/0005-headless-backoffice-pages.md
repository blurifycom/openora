...
Extract the OSS backoffice into `@openora/backoffice-ui`:
...
- **Types**: page components consume `z.infer<typeof Schema>` from `@openora/orpc-contract`. No local type duplication.

`apps/backoffice` in this repo is itself the first consumer - it imports from `@openora/backoffice-ui`. A consumer's `apps/web/app/admin/` is the second.
...
- Package: `packages/ui/backoffice/` (`@openora/backoffice-ui`).
- Adapter contracts already in place: `@openora/ui-provider-contract`, `@openora/ui-provider-daisyui` (ADR-0003).
