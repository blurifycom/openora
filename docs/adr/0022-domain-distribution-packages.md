...
ADR-0021 deliberately makes every feature a standalone, isolated `@blurifycom-addons/<name>` package. That isolation is load-bearing and is **not** changed here.
...
| ------------------------ | ----------------------------------------------------------------------------------------------- | ------------------------------ |
| `@blurifycom/platform`   | core, db, api-runtime, plugin-host, contracts aggregator, auth, i18n, audit, iam/RBAC, settings | -                              |
| `@blurifycom/account`    | identity, profile, player-management, compliance/RG                                             | players, KYC, limits           |
| `@blurifycom/wallet`     | wallet (fiat + crypto), transactions                                                            | payments, approvals            |
| `@blurifycom/casino`     | gaming, aggregator, lobby                                                                       | catalog, providers             |
| `@blurifycom/sportsbook` | sportsbook, odds adapter                                                                        | events, markets, risk          |
| `@blurifycom/engagement` | chat, notifications, bonus, friends, leaderboard, cms                                           | campaigns, content, moderation |
...
The six are thin **facade** packages. Source structure stays exactly as ADR-0021 (standalone `@blurifycom-addons/<name>` + platform packages, dependency-cruiser boundaries intact). Each facade:

- depends on its member packages via `workspace:*`,
- **bundles** them at build (members stay `private: true` and are never published; the facade's tarball is self-contained - `noExternal` the internal `@blurifycom/*` / `@blurifycom-addons/*`),
...
```ts
import { walletPlugin } from '@blurifycom/wallet/server'; // node
import { useWallet } from '@blurifycom/wallet/react'; // browser, headless hooks
import { walletSchemas } from '@blurifycom/wallet/contracts'; // isomorphic
import { walletAdmin } from '@blurifycom/wallet/server/admin'; // backoffice routes
```
...
- **Tier 1** - the six `@blurifycom/*` domain packages. Shared IP only. GitLab registry, later npm.
- **Tier 2** - everything proprietary (custom games, all custom UI, vendor adapters). Lives in the consumer repo / `@my-igaming/*` scope, its own cadence, **never** published to the OSS registry. Extends a domain through its existing plugin/adapter seams (ADR-0002, ADR-0014). Custom games plug into `@blurifycom/casino`'s game-integration seam (sessions, provably-fair, round balances - which is shared).

### 4. Domain topology

- `iam`/RBAC lives in `@blurifycom/platform` - access control is foundational, not a feature.
...
- `cms` sits in `@blurifycom/engagement` (content + marketing). Soft call; movable to platform.
- Domains are **siblings**: each depends only on `@blurifycom/platform`, never on each other. Cross-domain needs (eg casino debiting wallet) go through events / platform seams - same rule as ADR-0020.
...
- Consumer `.npmrc` maps `@blurifycom` to the GitLab project registry; auth token via env var. Registry versions are the **committed default**.
...
- **Public API is coarser** - consumers import `@blurifycom/casino` not `@blurifycom-addons/gaming`. Internal moves between member packages are non-breaking.
- **Consumer migration** - the consumer's imports and `pnpm-workspace.yaml` overrides change from ~15 fine-grained entries to six; stale entries (`@blurifycom/modules`, `react-hooks`, `sdk-core`, `ui-provider-*`) are dropped.
...
- Bundler choice + how migrations are surfaced to the consumer (single `@blurifycom/platform` migrate entry vs per-domain).
- Whether `@blurifycom/contracts` aggregation stays in platform or becomes its own published entry.
- Sportsbook concrete path (EveryMatrix OddsMatrix iframe vs custom frontend) affects how much of `@blurifycom/sportsbook` is real vs adapter-only.
