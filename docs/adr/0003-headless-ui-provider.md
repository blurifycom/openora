...
> **Superseded (2026-06-10):** The UI provider contract (`@blurifycom/ui-provider-contract`) and the shipped daisyUI adapter (`@blurifycom/ui-provider-daisyui`) were removed. The platform is now headless backend only - all UI lives in the downstream consumer. Modules no longer declare UI components, and no UI packages ship from this repo. Frontend development is entirely out-of-tree. This ADR is preserved as historical record of the exploration.
...
Define a **UI provider contract** in `@blurifycom/ui-provider-contract`: TypeScript interfaces for each component (Button, Form, Table, Dialog, etc.) and a union of named slot strings (`header-right`, `sidebar-bottom`, `user-menu`, ...).

Module UI pages (`packages/modules/<name>/ui/`) import only from `@blurifycom/ui-provider-contract`. The shipped implementation `@blurifycom/ui-provider-daisyui` (Tailwind v4 + DaisyUI) ships with the repo. Any adapter that satisfies the contract can be swapped in.
