...
Define a **UI provider contract** in `@openora/ui-provider-contract`: TypeScript interfaces for each component (Button, Form, Table, Dialog, etc.) and a union of named slot strings (`header-right`, `sidebar-bottom`, `user-menu`, ...).

Module UI pages (`packages/modules/<name>/ui/`) import only from `@openora/ui-provider-contract`. The shipped implementation `@openora/ui-provider-daisyui` (Tailwind v4 + DaisyUI) ships with the repo. Any adapter that satisfies the contract can be swapped in.
