...
The original decision below kept CORE add-on route contracts in `@openora/orpc-contract` (to avoid a core->add-on import) while only GATED add-ons owned an in-package `src/contract/` slice. That left a feature's contract in one package and its service/router in another - two locations for one source of truth.

This is now inverted so the model is uniform: **every add-on owns its contract in `src/contract/` (exported as `@openora-addons/<name>/contract`)** - core and gated alike, structurally identical. `@openora/orpc-contract` becomes a pure **aggregator**: it imports each core add-on's `/contract` slice, composes the single runtime `contract` the `@openora/react` typed client links against, and re-exports their schemas for back-compat. It owns only `health` (not an add-on). Gated add-on contracts still merge conditionally in `apps/api` (`editions.ts`), so the default typed client carries no gated surface.

Consequence: this inverts the dependency - `@openora/orpc-contract` (and transitively `@openora/react`) now depends on the core add-on packages. A narrow carve-out lets the aggregator import the read-only `@openora-addons/<name>/contract` subpath only (oxlint pins the exception to `orpc-contract`; dependency-cruiser exempts add-on `/contract` dirs). The cross-cutting `PlayerSchema` family moved to `@openora/shared-schemas` (it is shared by the `profile` and `player-management` add-ons). The count below is now **13 core add-ons** (localization was removed - translations live in the frontend consumer).

The original decision text is retained below for the record.

## Context

Previously the platform had two tiers: "modules" (14 features colocated in `packages/modules/<group>/<name>/`, sharing one `@openora/modules` package entry and central Drizzle migrations) and "add-ons" (4 optional `@openora-addons/<name>` packages with own migrations, gated by `OSS_ADDONS`). The group folders (`player`, `backoffice`, `platform`) existed only as organizational structure in prose; the codebase had no way to cleanly add a NEW feature to core or distinguish packaging at build time.

The distinction created friction: adding a feature meant deciding "module or add-on?" with no clear path forward; modules and add-ons followed different packaging rules (shared vs. standalone `package.json`); and the central `@openora/modules` package grew without bound while add-ons stayed frozen.

## Decision

**Every feature is a standalone `@openora-addons/<name>` package under `packages/addons/<name>/`** (own `package.json`, `tsconfig.json`, `src/{schema,schemas,service,router,plugin}`). The platform no longer has "modules" - only add-ons (or "features" in prose).

Two tiers **by registration**, not by packaging:

1. **CORE add-ons** (14 features) - registered in `extensions.config.ts` with **no `kind` attribute** - always load, contracts stay in `@openora/orpc-contract` (so the `@openora/react` typed client and `@openora/api-runtime` composition root don't import add-on packages), migrations in the central `packages/platform/db/drizzle.config.ts` (all core schemas under one history). This is the free/default build.
...
Consequence: the composition root (`@openora/api-runtime`) is now a **facade that knows about core add-on `/schema` subpaths ONLY** (for plugin host, DI container bootstrap). It does NOT import `@openora-addons/<name>` roots or any gated add-on. The platform builds in two editions:
...
- New features start as `@openora-addons/<name>` with own `package.json` (no need to decide "core or gated" upfront - default is core, opt-in to `kind: 'addon'` later).
...
- Add-ons import each other ONLY via read-only `/schema` subpaths: `import { wallet } from '@openora-addons/wallet/schema'`.
- Root imports: `@openora-addons/<name>` is forbidden in `packages/{platform,contracts,sdks}/**` and `@openora/api-runtime` (only allowed in `apps/*` composition roots, which load add-ons into `extensions.config.ts`).
...
- Dependency rules renamed:
  ...
  - New rule `no-core-to-addon` strictly forbids `packages/{platform,contracts,sdks}/**` importing `@openora-addons/*` (breaks the "composition root is the add-on wiring boundary" invariant).

### Scaffolding

- `pnpm gen module <name>` now generates `packages/addons/<name>/` as a standalone core add-on (registered in `extensions.config.ts` with no `kind`, contract in `@openora/orpc-contract`).
...
- The reverse (gated → core) involves moving migrations back to the central history, moving the contract to `@openora/orpc-contract`, and registering with no `kind` - again, no module logic change.
...
- **Decoupling core composition.** `@openora/api-runtime` does not import add-on packages (only their schemas, via subpath), so adding a new add-on does NOT trigger a rebuild of the composition root.
