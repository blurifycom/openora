...
1. All feature modules compiled into the single `@openora/modules` package - an add-on
   module could not be a paywalled subpath.
2. The root contract `@openora/orpc-contract` statically imported every module's slice,
...
`packages/addons/<name>/` named `@openora-addons/<name>`, shaped exactly like its
eventual published form:

- Own `package.json` + `tsconfig.json` (extends `@openora/tsconfig/node-service.json`).
- Own contract slice in `src/contract/` (moved OUT of `@openora/orpc-contract`); the
...
Add-on modules may import core (`@openora/contracts/*`, `@openora/adapters`,
`@openora/platform/*`, `@openora/modules/<g>/<n>/schema`) exactly like a core module. They
...
- `no-core-to-addon`: nothing under `packages/{modules,platform,contracts,sdks}/`
  may import `@openora-addons/*`. Only composition roots under `apps/*` may.
...
The free core contract (`@openora/orpc-contract`) carries no add-on slice. The
...
- The `player` table + the self-profile surface moved to a new **core** module
  ...
  stay in core `@openora/orpc-contract`.
- The admin surface moved to `@openora-addons/player-management`, which reads the core
  `player` table via `@openora/modules/player/profile/schema` (add-on -> core) and owns
...
- The core SDK (`@openora/react`) no longer ships add-on hooks/prefetchers (PAM
