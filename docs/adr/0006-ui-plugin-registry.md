...
> lived in `@openora/react-sdk`, which was deleted when the platform went headless (see the
...
> consumer repo and `@openora/react` is the only supported consumption surface. A
...
Each page in `@openora/react-sdk` reads from the registry at render time via internal hooks (`useNavItems`, `useUserColumns`, etc.) and renders the registered contributions inline alongside its own defaults.
...
1. Extending the `UIPluginContext` interface in `@openora/react-sdk/src/ui-plugin/context.ts`.
...
import { renderRegisteredRoute } from '@openora/react-sdk';
...
- Plugins extend the admin without forking. The promise of "extend without changing @openora/react-sdk" becomes real.
