...
(`getUserId` in `@openora/core`), and likewise the tenant from `x-tenant-id`
...
**1. One shared verified-session resolver.** `SessionResolver` (`@openora/auth`,
...
and throw `ORPCError('UNAUTHORIZED')` when absent. `@openora/core` cannot import `@openora/auth`
...
- `@openora/core` stays free of an `@openora/auth` dependency - the verified identity is passed
