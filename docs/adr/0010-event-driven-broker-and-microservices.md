...
> **Update (2026-07-25)**: [ADR-0039](./0039-tests-run-the-production-seams.md) deleted `InMemoryBroker` outright - the test suite binds `RedisStreamsBroker` too, so the "survives as a test-only double" clause above no longer holds. Nothing else in this ADR changes.
