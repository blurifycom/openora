...
**Status**: Accepted. The `ChatTransportPort` is realized as the generic `REALTIME_TRANSPORT` seam (`@openora/adapters` `realtime.ts`), with a first-party in-process default and the `chat` module streaming live over SSE - see ADR-0014. Economy commands (`/tip`/`/rain`/`/gift`) remain open.
