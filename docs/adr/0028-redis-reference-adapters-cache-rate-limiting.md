...
> **Update (2026-07-25)**: [ADR-0039](./0039-tests-run-the-production-seams.md) deleted `InProcessCache`/`InProcessRateLimiter` and ended the zero-infra path - `bootTestApp` binds `RedisCache`/`RedisRateLimiter`, so Redis is now a prerequisite for the suite. The driver choice and the semantics below are unchanged.
