...
registry, layered react SDK packages (`@openora/react-sdk` → `@openora/react-pages` /
`@openora/react-blocks`), reference apps (`apps/web`, `apps/backoffice`), and a player-front
...
seams, and a single **headless** consumption SDK (`@openora/react`: data hooks, auth,
...
consume the OSS API over HTTP through the typed oRPC client in `@openora/react`.

Consequences for the removed surface:

- `@openora/react-sdk`, `@openora/react-pages`, `@openora/react-blocks`, `@openora/ui-provider-*`, and
  the reference `apps/web` / `apps/backoffice` are removed. `@openora/react` is the only
...
headless API and `@openora/react` contract stay valid either way.
