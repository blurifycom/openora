...
- Frontend platform (historical): Consumer `apps/web` consumes the OSS admin at `/admin/*`; headless backoffice + typed client + theme system consolidated into the React SDK packages; UI plugin registry (ADR-0006) shipped with `vip-tiers` as first consumer; Storybook refactored to contract-driven + adapter switcher; Docker reference stack + one-command `pnpm setup` for Consumer; end-to-end auth flow (register/login/cookie session/logout) working. SINCE SUPERSEDED (2026-06-09): the page/block SDK layer (`@oss/react-pages` / `@oss/react-blocks`) and the reference frontend apps (`apps/web` / `apps/backoffice`) were removed from the OSS repo. The platform is now headless; the frontend lives in consumer and will be re-extracted later. `@oss/react-hooks` + `@oss/sdk-core` remain the supported frontend consumption surface.
