...
flowchart TB
  ...
  subgraph DCOMP["@openora/core/compliance · RG/KYC"]
    COMP["compliance ⟨country_rule·user_limit⟩"]:::core
  end
...
Generated from `docs/catalog.json` - 20 modules, 265 routes, 45 adapter ports, 117 events. Edit the code, then run `pnpm gen:catalog`.
...
| `@openora/core/compliance`    | compliance                                                 | country_rule, global_kyc_config, kyc_verification, rg_exclusion + 2 more            | 30     |
...
| `@openora/core/pam`           | identity · player-management · player-note · profile · tag | account, admin_trusted_device, phone_verification_session, player + 9 more          | 65     |
