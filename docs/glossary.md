...
| ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| **Headless**                             | The platform ships backend only - modules, contracts, API, and SDK consumption surface. The operator supplies the entire frontend (pages, components, styling, theme) in their own repo.                                                            | `packages/domains/*`, `@oss/core/react`                                           |
...
| **Adapter**                              | The vendor-agnostic interface a module depends on (e.g. `KycAdapter`, `PaymentAdapter`) plus its per-vendor implementations. The swap seam: a module declares the interface + DI token in `@oss/core/contracts`; an operator binds a concrete impl. | `@oss/core/contracts` + `modules/<m>/adapters/<vendor>/`                          |
| **Contract**                             | The composed oRPC router. Drives request validation, the typed client, and the emitted OpenAPI spec.                                                                                                                                                | `@oss/core/contracts`                                                             |
| **Domain schema**                        | A Zod schema - the single source of truth for a shape. Types are `z.infer`'d, never hand-written.                                                                                                                                                   | `@oss/core/contracts`, module `schemas/`                                          |
...
| **Sealed token**                         | A `SealedToken<T>` whose backing service operators may never override (RG enforcement, KYC writes, AML/SAR, ledger writes, RNG, etc.). Structurally incompatible with `Token<T>` + runtime-rejected by plugin-host.                                 | `@oss/core/compliance`                                                            |
| **Client page token**                    | `ClientPageToken<P>` - Tier 3 escape hatch for full client-side page replacement.                                                                                                                                                                   | `@oss/core/contracts/token.ts`                                                    |
...
| **Platform config**                      | Operator-editable Zod-validated YAML/JSON consumed at boot. Controls feature flags, brands, RG defaults per geo. No admin UI in v1.                                                                                                                 | `@oss/core/contracts/platform-config.ts`, `@oss/core/server (loadPlatformConfig)` |
| **Tenant** / **multi-tenant**            | A logical isolation boundary; every scoped row carries `tenantId` and is read via `withTenant`.                                                                                                                                                     | `@oss/core/server`                                                                |
