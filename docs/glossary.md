...
| Term                                                                 | Who it is                                                                                                                                                                                                                                       |
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Operator** / **consumer** / **downstream consumer** / **end user** | The developer or company that installs the `@blurifycom/*` packages and builds + runs their own igaming. They are the "end user" of the _package_, not a gambler. They bring their own `extensions.config.ts`, UI adapter, and vendor adapters. |
...
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| **Headless**                             | The platform ships backend only - modules, contracts, API, and SDK consumption surface. The operator supplies the entire frontend (pages, components, styling, theme) in their own repo.                                                                   | `packages/domains/*`, `@blurifycom/core/react`                                                  |
| **Module**                               | A business domain packaged as an independently loadable unit (auth, wallet, gaming...). Never imports another module.                                                                                                                                      | `packages/domains/<name>/`                                                                      |
...
| **Adapter**                              | The vendor-agnostic interface a module depends on (e.g. `KycAdapter`, `PaymentAdapter`) plus its per-vendor implementations. The swap seam: a module declares the interface + DI token in `@blurifycom/core/contracts`; an operator binds a concrete impl. | `@blurifycom/core/contracts` + `modules/<m>/adapters/<vendor>/`                                 |
| **Contract**                             | The composed oRPC router. Drives request validation, the typed client, and the emitted OpenAPI spec.                                                                                                                                                       | `@blurifycom/core/contracts`                                                                    |
| **Domain schema**                        | A Zod schema - the single source of truth for a shape. Types are `z.infer`'d, never hand-written.                                                                                                                                                          | `@blurifycom/core/contracts`, module `schemas/`                                                 |
...
| **Sealed token**                         | A `SealedToken<T>` whose backing service operators may never override (RG enforcement, KYC writes, AML/SAR, ledger writes, RNG, etc.). Structurally incompatible with `Token<T>` + runtime-rejected by plugin-host.                                        | `@blurifycom/core/compliance`                                                                   |
| **Client page token**                    | `ClientPageToken<P>` - Tier 3 escape hatch for full client-side page replacement.                                                                                                                                                                          | `@blurifycom/core/contracts/token.ts`                                                           |
...
| **Platform config**                      | Operator-editable Zod-validated YAML/JSON consumed at boot. Controls feature flags, brands, RG defaults per geo. No admin UI in v1.                                                                                                                        | `@blurifycom/core/contracts/platform-config.ts`, `@blurifycom/core/server (loadPlatformConfig)` |
