...
- **`exposesReporting`** (optional) - set when the result reveals reporting data an admin with only `game-config:view` must not infer, such as a revenue ranking. Previewing, saving in rule mode, or re-evaluating a rule with such a clause needs `report:view` on top of the route's own `game-config` permission. No built-in sets it: `most_played` shows only which games are popular, with no figures, and that is what the resulting category shows players anyway.
...
`POST`/`PATCH` on a category also accept `membershipMode` and `membershipRule`. The preview needs `game-config:view`. The preview, a create or update that puts a category in rule mode or changes its rule, and an on-demand evaluation also need `report:view` when a clause's definition sets `exposesReporting` - none of the built-ins does.
