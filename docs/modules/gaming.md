...
Each membership evaluation reads the category's PostgreSQL row version before resolving its rule. It may write membership or failure status only while that version is unchanged. A completed evaluation or a configuration update invalidates older snapshots, including a change away from and back to the same configuration. A stale result is discarded and retried from current data, at most three times. Unrelated category or ranking updates can also cause a retry. The [database-provided row version](https://www.postgresql.org/docs/current/ddl-system-columns.html) is used only during an evaluation; no additional column or persistent version counter is needed.
