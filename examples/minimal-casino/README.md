...
that boots a real casino API on top of the `@oss/*` packages without forking core.
...
- `src/main.ts` - boot the API with `createApp({ plugins, casino, port, cors, openapi })`.
...
- This `casino-oss` repo checked out as a sibling of your consumer repo, eg:
  ```
  parent/
    casino-oss/        <- this repo
    my-casino/         <- a copy of examples/minimal-casino
  ```

## Run steps

1. Copy this folder out next to `casino-oss` and install. The `link:` paths in
   `package.json` resolve relative to the consumer dir, so a sibling layout is
   assumed (`../casino-oss/...`).
   ```bash
   cp -r casino-oss/examples/minimal-casino my-casino
   cd my-casino
   pnpm install
   ```
