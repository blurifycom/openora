---
description: >-
  Run the full pre-PR gate locally - `pnpm verify` plus the drift check CI runs
  (`pnpm check:drift`), which `pnpm verify` alone does NOT cover. Catches stale
  catalog/agent-docs before push.
---
