...
The finding carries the deposit's own `amount`, `currency` and `transactionId` as columns. The limit side - which limit, which period, how much of the window is used - is prose in `detail`, because the table has no metadata column; it is readable, not queryable. A retried webhook re-files a missing finding, and the finding dedup index keeps the retry from duplicating one.

That dedup index is now `(kind, providerName, externalId)` rather than `(kind, externalId)`. A vendor reference is only unique within its own vendor, so under the old key two providers handing out the same id meant the second one's finding was silently dropped as a duplicate - for every finding kind, not only this one. The reconciliation cycle's already-reported checks match the provider for the same reason.

A wallet loaded without `RG_LIMITS` bound now logs one warning at startup saying deposit limits are not enforced, instead of leaving every deposit route silently ungated.
