...
Fixed a race in the withdrawal address book's 50-address-per-player cap: concurrent saves could each read the count before any of them wrote, letting more than 50 rows land for one player. The count-and-insert is now one statement, serialized per player with a transaction-scoped advisory lock, so the cap holds under concurrent writers. A rejected save now correctly distinguishes "the cap is full" from "this exact address is already saved".
